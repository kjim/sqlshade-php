<?php
require_once(dirname(__FILE__).'/TokenStream.php');
...
class SQLShade_Lexer
{
    ...
    public function __construct($env = null, array $options = array())
    {
        if (null !== $env) {
    ...
    public function tokenize($code, $filename = 'n/a')
    {
        ...
        while (!$end) {
            ...
            $end = $token->getType() === SQLShade_Token::EOF_TYPE;
        }
        ...
        return new SQLShade_TokenStream($tokens, $this->filename);
    }
    ...
    protected function nextToken()
    {
        ...
        if ($this->cursor >= $this->end) {
            return new SQLShade_Token(SQLShade_Token::EOF_TYPE, '', $this->lineno);
        }
    ...
    protected function lexData()
    {
        ...
        if (false === $pos1 && false === $pos2 && false === $pos3) {
            $rv = new SQLShade_Token(SQLShade_Token::TEXT_TYPE, substr($this->code, $this->cursor), $this->lineno);
        ...
        if (!empty($text)) {
            $result[] = new SQLShade_Token(SQLShade_Token::TEXT_TYPE, $text, $lineno);
        ...
        switch ($token) {
            case $this->options['tag_comment'][0]:
                if (!preg_match('/(.*?)'.preg_quote($this->options['tag_comment'][1], '/').'/As', $this->code, $match, null, $this->cursor)) {
                    throw new SQLShade_SyntaxError('unclosed comment', $this->lineno, $this->filename);
                }
            ...
            case $this->options['tag_block'][0]:
                ...
                if (preg_match('/\s*raw\s*'.preg_quote($this->options['tag_block'][1], '/').'(.*?)'.preg_quote($this->options['tag_block'][0], '/').'\s*endraw\s*'.preg_quote($this->options['tag_block'][1], '/').'/As', $this->code, $match, null, $this->cursor)) {
                    $result[] = new SQLShade_Token(SQLShade_Token::TEXT_TYPE, $match[1], $lineno);
                    $this->moveCursor($match[0]);
                    $this->moveLineNo($match[0]);
                    $this->position = self::POSITION_DATA;
                } else {
                    $result[] = new SQLShade_Token(SQLShade_Token::BLOCK_START_TYPE, '', $lineno);
                    $this->position = self::POSITION_BLOCK;
                }
                break;

            case $this->options['tag_variable'][0]:
                $result[] = new SQLShade_Token(SQLShade_Token::VAR_START_TYPE, '', $lineno);
    ...
    protected function lexBlock()
    {
        if (preg_match('/\s*'.preg_quote($this->options['tag_block'][1], '/').'/As', $this->code, $match, null, $this->cursor)) {
            ...
            return new SQLShade_Token(SQLShade_Token::BLOCK_END_TYPE, '', $lineno);
        }
    ...
    protected function lexVar()
    {
        if (preg_match('/\s*'.preg_quote($this->options['tag_variable'][1], '/').'/As', $this->code, $match, null, $this->cursor)) {
            ...
            return new SQLShade_Token(SQLShade_Token::VAR_END_TYPE, '', $lineno);
        }
    ...
    protected function lexExpression()
    {
        ...
        // sanity check
        if ($this->cursor >= $this->end) {
            throw new SQLShade_SyntaxError('Unexpected end of stream', $this->lineno, $this->filename);
        }
        ...
        if (preg_match(self::REGEX_OPERATOR, $this->code, $match, null, $this->cursor)) {
            ...
            return new SQLShade_Token(SQLShade_Token::OPERATOR_TYPE, $match[0], $this->lineno);
        }
        // now names
        else if (preg_match(self::REGEX_NAME, $this->code, $match, null, $this->cursor)) {
            $this->moveCursor($match[0]);

            return new SQLShade_Token(SQLShade_Token::NAME_TYPE, $match[0], $this->lineno);
        }
        // then numbers
            ...
            return new SQLShade_Token(SQLShade_Token::NUMBER_TYPE, $value, $this->lineno);
        }
        // and finally strings
            ...
            return new SQLShade_Token(SQLShade_Token::STRING_TYPE, $value, $this->lineno);
        }

        // unlexable
        throw new SQLShade_SyntaxError(sprintf("Unexpected character '%s'", $this->code[$this->cursor]), $this->lineno, $this->filename);
    }
