...
class SQLShade_TokenStream
{
    ...
    public function next($fromStack = true)
    {
        ...
        if (null === $token) {
            throw new SQLShade_SyntaxError('Unexpected end of template', -1);
        }

        // trim blocks
        if ($this->trimBlocks &&
            $this->current &&
            SQLShade_Token::BLOCK_END_TYPE === $this->current->getType() &&
            SQLShade_Token::TEXT_TYPE === $token->getType() &&
            $token->getValue() &&
        ...
        $this->eof = $token->getType() === SQLShade_Token::EOF_TYPE;
    ...
    public function expect($primary, $secondary = null)
    {
        ...
        if (!$token->test($primary, $secondary)) {
            throw new SQLShade_SyntaxError(sprintf('Unexpected token "%s" of value "%s" ("%s" expected%s)',
                SQLShade_Token::getTypeAsString($token->getType()), $token->getValue(),
                SQLShade_Token::getTypeAsString($primary), $secondary ? sprintf(' with value "%s"', $secondary) : ''),
